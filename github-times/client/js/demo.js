(function() {
	...
	function init() {
		...
		bespoke.on('activate', function(event){
			var title = event.slide.getAttribute('data-title');
			document.getElementById('headline').innerHTML = '<a>' + title + '</a>';
			draw(event.index);
			return false;
		});

		bespoke.on('deactivate', function(event){
			if (event.index != 0){
				clear(event.slide.getAttribute('id'));
			}
			return false;
		});
	}
	...
	function initButtons() {
		...
		document.getElementById('left-arrow').addEventListener('click', prev);
